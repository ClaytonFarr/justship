    ...
    Container
      nav.flex.h-20.items-center.justify-between.gap-x-8.py-6.lg_py-8(aria-label='Global')
        ...
        .hidden.md_flex.md_gap-x-8
          +each('navigation as item')
            a.text-sm.font-medium.leading-6.text-slate-900(href='{PUBLIC_ORIGIN}{ item.href }') {item.label}

        +if('$page.data.user')
          .flex.flex-1.justify-end
            DropdownMenu(menuLabel='Account', menuItems='{ accountNavigation }')
              form.group.flex.w-full.cursor-pointer.items-center.gap-x-3.rounded-md.px-6.py-3.pr-16.hover_bg-slate-50(
                method='post',
                action='{PUBLIC_ORIGIN}/signout',
                use:enhance
              )
                LogOut.h-4.w-4.opacity-50.transition.duration-200.group-hover_opacity-100
                button.text-left.text-sm.font-medium.leading-6.text-slate-900.group-hover_text-slate-900(type='submit') Sign Out
          +else
            .flex.flex-1.items-center.justify-end.gap-x-7
              a.hidden.whitespace-nowrap.lg_block.lg_text-sm.lg_font-medium.lg_leading-6.lg_text-slate-900(href='{PUBLIC_ORIGIN}/signin') Sign In
      ...
      +if('mobileMenuOpen')
        .md_hidden(role='dialog', aria-modal='true')
          .fixed.inset-0.z-10
          .fixed.inset-y-0.right-0.z-10.w-full.overflow-y-auto.bg-white.px-6.py-6.sm_max-w-sm.sm_ring-1(class='sm_ring-slate-900/10')
            .flex.items-center.justify-between.gap-x-6
              h1: a.flex.items-center.gap-5(href='{PUBLIC_ORIGIN}/', class='-m-1.5 p-1.5')
                img.h-8.w-auto(src='logo.svg', alt='')
                span.text-lg.font-semibold.text-slate-900.sm_sr-only {PUBLIC_PROJECT_NAME}
              +if('!$page.data.user')
              ...
              button.rounded-md.text-slate-700(type='button', class='-m-2.5 p-2.5', on:click!='{ () => mobileMenuOpen = false }')
                span.sr-only Close menu
                X.h-6.w-6.opacity-50.transition.duration-150.hover_opacity-100(strokeWidth='1.5', aria-hidden='true')
            .mt-6.flow-root
              .-my-6.divide-y(class='divide-slate-500/10')
                .space-y-2.py-6
                  +each('navigation as item')
                    a.-mx-3.block.rounded-lg.px-3.py-2.text-base.font-medium.leading-7.text-slate-900.hover_bg-slate-50(href='{PUBLIC_ORIGIN}{ item.href }') {item.label}
                .py-6
                  form.group.-mx-3.flex.cursor-pointer.items-center.gap-x-3.rounded-md.px-3.py-2.hover_bg-slate-50(
                    method='post',
                    action='{PUBLIC_ORIGIN}/signout',
                    use:enhance
                  )
