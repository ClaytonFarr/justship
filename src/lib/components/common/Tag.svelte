<script lang="ts">
  ...
  let styleClasses = {
    ...
    bordered: `rounded-full px-4 py-1 ring-1 ring-rule ${
      reversed ? 'text-content-secondary-reversed' : 'text-content-secondary'
    }`,
  }
...
<template lang="pug">
  +if('href')
    ...
    a(href='{ href }', class='{ tagClasses }').group
      +if('iconLeft')
        svelte:component.h-5.w-5(this='{ iconLeft }')
      span {label}
      +if('iconRight')
        svelte:component.h-5.w-5(this='{ iconRight }')
    ...
    +else
      ...
      span(class='{ tagClasses }')
        ...
        span {label}
        +if('iconRight')
          svelte:component.h-5.w-5(this='{ iconRight }')</template>
