      ...
      nav.flex.h-20.items-center.justify-between.gap-x-6.py-6.sm_gap-x-10.lg_py-8(aria-label='Global')
        ...
        .flex(class!='{ navAlignment === "center" ? "flex-1" : "flex-1 sm_flex-0" }')
          h1: a.flex.items-center.gap-4(href='/', class='-m-1.5 p-1.5')
            ...
            span.font-display.text-lg.text-xl.font-medium.tracking-tight.text-foreground {PUBLIC_PRODUCT_NAME}
        // prettier-ignore
        +if('navItems.length > 0')
          .md_gap-x-10(class!='{navItems.length > 1 ? "hidden md_flex" : "flex"} { navAlignment === "left" ? "justify-start flex-1" : navAlignment === "center" ? "justify-center" : "justify-end flex-1" }')
            +each('navItems as item')
              a.text-sm.font-medium.leading-6.text-foreground.hover_text-action.transition(href='{ item.href }') {item.label}
        +if('cta.include')
          // prettier-ignore
            ...
            .mt-6.flow-root
              +if('navItems.length > 0')
                .-my-6.divide-y(class='divide-border/10')
                  .space-y-2.py-6
                    +each('navItems as item')
                      a.hover_bg-muted-50.-mx-3.block.rounded-lg.px-3.py-2.text-base.font-medium.leading-7.text-foreground(
                        href='{ item.href }'
                      ) {item.label}
                +if('cta.include')
                  .py-8
