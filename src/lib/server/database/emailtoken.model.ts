...
export const deleteAllEmailTokensForUser = async (userId: string) => {
  await db.delete(emailVerificationTokenTable).where(eq(emailVerificationTokenTable.user_id, userId))
}

export const createEmailVerificationToken = async (userId: string, email: string): Promise<string> => {
...
export const getEmailToken = async (tokenId: string) => {
  const token = await db.select().from(emailVerificationTokenTable).where(eq(emailVerificationTokenTable.id, tokenId))
  if (token.length === 0) {
    return null
  } else {
    return token[0]
